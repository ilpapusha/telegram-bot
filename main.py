...
# Только https-ссылка на username, tg://user?id=... не используем (бывает BUTTON_USER_PRIVACY_RESTRICTED)
...
KITS = [
 {"code":"vostochny","title":"🕌🌙 Набор «Восточный»",
  "photo":"AgACAgIAAxkBAAN_aNF8NzIeIItUw9J4P3oLvcshM7wAAsf2MRtqspFKEp0AAee2XsX8AQADAgADeQADNgQ",
  "items":[("Montale Arabians Tonka",5),("Mancera Red Tobacco",5),("Parfums de Marly Althair",5),("Azzaro The Most Wanted Parfum",5),("Armani Stronger With You Absolutely",5)]},

 {"code":"svezhiy","title":"💦🍃 Набор «Свежий»",
  "photo":"AgACAgIAAxkBAAN9aNF8GK0h-cwWMHP6WkAlFwABQmwZAALF9jEbarKRShK5MEgD8MXBAQADAgADeQADNgQ",
  "items":[("Dior Homme Cologne",5),("Parfums de Marly Greenley",5),("Prada L’Homme L’Eau",5),("Armani Acqua Di Gio Profondo Parfum",5),("Jean Paul Gaultier Le Beau Le Parfum",5)]},

 {"code":"vecherniy","title":"🌙✨ Набор «Вечерний»",
  "photo":"AgACAgIAAxkBAAN4aNF5HwqkdgNcQjgg9gABSo25lFxfAAKw9jEbarKRSqr-Vm7eaG3mAQADAgADeQADNgQ",
  "items":[("Armaf Club de Nuit Intense",5),("Jean Paul Gaultier Le Male Le Parfum",5),("Chanel Bleu de Chanel Eau de Parfum",5),("Yves Saint Laurent Myslf Eau de Parfum",5),("Tom Ford Noir",5)]},

 {"code":"komplimentarnyy","title":"💘🌟 Набор «Комплиментарный»",
  "photo":"AgACAgIAAxkBAAOCaNF8eAd67W7XRioTneRMlxlJSb4AAsr2MRtqspFKaGxSSlgviZYBAAMCAAN5AAM2BA",
  "items":[("Initio Side Effect",5),("Armaf Club de Nuit Intense",5),("Jean Paul Gaultier Le Male Le Parfum",5),("Prada L’Homme L’Eau",5),("Yves Saint Laurent La Nuit de L’Homme Eau de Parfum",5)]},

 {"code":"na_vse_sluchai","title":"🎯🧩 Набор «На все случаи жизни»",
  "photo":"AgACAgIAAxkBAAOEaNF8i5lVXVryRVLRgEMDH8EpLl8AAsv2MRtqspFKosf2SsGkXIoBAAMCAAN5AAM2BA",
  "items":[("Montale Arabians Tonka",5),("Armaf Club de Nuit Intense",5),("Armani Acqua Di Gio Parfum",5),("Jean Paul Gaultier Le Male Le Parfum",5),("Chanel Bleu de Chanel Eau de Parfum",5)]},

 {"code":"big_g","title":"🦁💥 Набор «BIG G»",
  "photo":"AgACAgIAAxkBAAOGaNF8nCTpNhq-UNWry9jtTr8mTnAAAsz2MRtqspFKDfNtpsgRR90BAAMCAAN5AAM2BA",
  "items":[("Parfums de Marly Layton",5),("Tom Ford Ombre Leather",5),("Azzaro The Most Wanted Parfum",5)]},

 {"code":"dzhentelmen","title":"🤵🎩 Набор «Джентельмен»",
  "photo":"AgACAgIAAxkBAAOIaNF8tAfhnzZUEA5IVrXE18KM9L8AAs32MRtqspFK5w3gcQQrVvMBAAMCAAN5AAM2BA",
  "items":[("Parfums de Marly Sedley",5),("Prada L’Homme",5),("Jean Paul Gaultier Le Male Le Parfum",5)]},

 {"code":"papochka","title":"👑🔥 Набор «Папочка»",
  "photo":"AgACAgIAAxkBAAOKaNF8yBFmwnJyKoc4jSiQXkYEbgsAAs72MRtqspFK2Y3owjZIH4cBAAMCAAN5AAM2BA",
  "items":[("Parfums de Marly Althaïr",5),("Dior Homme Intense",5),("Mancera Red Tobacco",5)]},
]
...
DECANT_BRANDS = [
 ...
 {"brand":"Armani","items":[
  ...
  {"code":"armani_acqua_di_gio_profondo_parfum","title":"Armani Acqua Di Gio Profondo Parfum","photo":"AgACAgIAAxkBAAOtaNGV1t9md2a0AuBpubS3oEznDDoAAhP4MRtqspFKZwIITqOJT4cBAAMCAAN5AAM2BA","desc":"Глубокий акватический аккорд с минеральностью.","prices":{5:1200,8:1850,18:3950}},
 ]},
 {"brand":"Azzaro","items":[
...
# [(chat_id, message_id), ...] — храним экраны бота; последнее оставляем, остальное чистим
...
def decant_kb(bi:int, pi:int, prices:dict) -> KB:
    # На кнопках только объёмы (без цен)
...
# Пытаемся редактировать только сообщения бота; иначе — отправляем новое
...
async def show_brand(c: types.CallbackQuery):
    ...
    # Не удаляем текущее сообщение — чат не пустеет
...
async def on_text(m: types.Message):
    ...
    # Связаться с продавцом через бота (если нет SELLER_URL)
...
async def del_menu(c: types.CallbackQuery):
    ...
    # Кнопки удаления набора целиком
...
async def contact_seller(c: types.CallbackQuery):
    uid = c.from_user.id
    if SELLER_URL:
        # Публичный username задан — даём прямую ссылку
        return await c.message.answer(f"Свяжитесь с продавцом: {SELLER_URL}")
    # Иначе — собираем текст у клиента и шлём админу
